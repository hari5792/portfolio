...
app.get('/health', (req, res) => {
  res.json({ service: 'Parser Microservice', status: 'UP', port: PORT, timestamp: new Date().toISOString() });
});
  ...
  try {
    ...
    if (req.file) {
      ...
      if (fileName.endsWith('.xlsx') || fileName.endsWith('.xls')) {
        result = parseExcelBuffer(req.file.buffer);
      } else {
        const text = req.file.buffer.toString('utf-8');
        result = parseCSVText(text);
      }
    } else if (req.body.csvText) {
      result = parseCSVText(req.body.csvText);
    } else {
      return res.status(400).json({ error: 'No file or CSV text provided in request body.' });
    }
    ...
    console.error('[Parser Microservice Error]:', err.message);
...
app.listen(PORT, () => {
  console.log(`[Parser Microservice] Running on http://localhost:${PORT}`);
});
